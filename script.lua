...
local function createMM2Gui()
	...
	task.spawn(function()
		while true do
				...
				if not coinFarmPlatform or not coinFarmPlatform.Parent then
					...
					platformHeartbeat = RunService.Heartbeat:Connect(function()
						...
						if char then
							...
							if h then
								if h.Anchored then
									-- Player is being tweened; follow normally so platform stays right under them
									platformFloorY = h.Position.Y - 3.5
								else
									-- Player is unanchored (collecting coins); lock platform Y so it catches them if they fall
			...
			-- Clean up visited list for coins that no longer exist
			...
			-- Only tween and collect if farming is active
			if not mm2CoinFarming then
				task.wait(0.2)
				continue
			end

			-- Check for murderer and pick nearest safe unvisited coin
			...
			-- If all coins visited or unsafe, reset and try again
			if not targetCoin then
				...
				-- If still no safe coin, wait and retry
				if not targetCoin then
					task.wait(1)
					continue
				end
			...
			-- Verify the target coin is still there before going for it
			...
			-- Calculate tween duration based on distance and speed setting
			...
			-- Wait for tween with timeout (prevents hanging if character dies mid-tween)
			...
			-- If character died/respawned during tween, skip and restart loop
			...
			-- If murderer got close during tween, skip pickup and move on
			local mPos = getMurdererPosition()
			...
			-- Unanchor so the player can physically touch and collect the coin
			hrp.Anchored = false

			-- Wait for the coin to be picked up (it disappears when collected)
			-- Timeout after 5 seconds so we don't get stuck on unreachable coins
			-- If not collected after 1.5 seconds, move the player slightly up
			local pickupTimeout = 5
			local elapsed = 0
			local movedUp = false
			while targetCoin.Parent and mm2CoinFarming and elapsed < pickupTimeout do
				-- If murderer gets close, abandon this coin and flee
				local mPos2 = getMurdererPosition()
				if mPos2 and (hrp.Position - mPos2).Magnitude < safeDistance then
					break
				end
				task.wait(0.1)
				elapsed += 0.1
					-- After 1.5 seconds without pickup, nudge the player up slightly
			...
			-- Mark this coin as visited so we move to a different one next time
			visitedCoins[targetCoin] = true
			...
			-- Freeze the player briefly after picking up the coin
			if mm2CoinFarming then
	...
	-- Gun farm loop (similar to coin farm)
	task.spawn(function()
		while true do
			local character = localPlayer.Character
			if not character then
			...
			-- Unanchor so the player can physically pick up the gun
			hrp.Anchored = false

			-- Wait for the gun to be picked up (it disappears when collected)
			local pickupTimeout = 5
			local elapsed = 0
			local movedUp = false
			...
			-- Brief freeze after picking up the gun
...
local function lockOnNearest()
	...
	-- Only include targets within LOCK_RANGE
	targetList = {}
...
-- Comprehensive animation ID list organized by category
local animCategories = {
	{title = "=== R15 DEFAULT ANIMATIONS ===", items = {
		{"Run", "10921148209"}, {"Walk", "10921152678"}, {"Jump", "10921308158"},
		{"Idle 1", "10921301576"}, {"Idle 2", "10921302207"}, {"Fall", "10921307241"},
...
local function createMM2Gui()
	...
	-- Handle character respawns: cancel active tween, clean up, and reset state
	localPlayer.CharacterAdded:Connect(function()
		if coinTween then
			coinTween:Cancel()
			coinTween = nil
	...
	-- Find the murderer (person with knife) position
	local function getMurdererPosition()
		for _, player in ipairs(Players:GetPlayers()) do
			if player ~= localPlayer then
				local char = player.Character
	...
	task.spawn(function()
		while true do
			...
			-- Manage invisible platform at player's feet to prevent falling
			if mm2CoinFarming then
				if not coinFarmPlatform or not coinFarmPlatform.Parent then
					coinFarmPlatform = Instance.new("Part")
					coinFarmPlatform.Name = "VlexCoinFarmPlatform"
